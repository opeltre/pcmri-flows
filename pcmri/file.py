...
from revert import resample
import pcmri.dataset as dataset
...
class File:
    ...
    def __init__(self, exam_path):
        ...
        for name in os.listdir(self.path):
            ...
            if key in dataset.CHANNELS \
               and dataset.CHANNELS[key]["type"] != "x":
    ...
    def sumAll (self, fluxtype, fields=['debit'], fmt='torch'):
        patterns = [c for c in self.channels 
                      if dataset.CHANNELS[channel(c)]['type'] == fluxtype] 
        print(patterns)
    ...
    def readAll (self, *patterns, fields=['debit'], fmt='torch'):
        ...
        for f in files:
            ...
            if len(flux) > 0 and dataset.CHANNELS[key] != 'x':
